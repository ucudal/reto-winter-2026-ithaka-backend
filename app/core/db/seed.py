...
def seed_database(force: bool = False):
    ...
    try:
        ...
        meetings = [
            Meeting(group_id=groups[0].id, tutor_ids=[t_b1.id, t_t1.id], date=datetime(2026, 4, 10, 15, 0, tzinfo=timezone.utc), participants=[students[0].id, students[1].id], notes="Propuesta de valor y ruteo.", next_steps="Ajustar BMC B2B.", hours_spent=2.5, links=[{"type": "Drive", "url": "https://drive.google.com/ecoroute-minuta-1"}]),
            Meeting(group_id=groups[1].id, tutor_ids=[t_b2.id], date=datetime(2026, 4, 12, 10, 0, tzinfo=timezone.utc), participants=[students[2].id, students[3].id], notes="Hipótesis de clientes.", next_steps="Realizar 5 entrevistas.", hours_spent=1.5, links=[{"type": "Drive", "url": "https://drive.google.com/healthpulse-minuta-1"}]),
            Meeting(group_id=groups[2].id, tutor_ids=[t_t1.id], date=datetime(2026, 4, 15, 16, 0, tzinfo=timezone.utc), participants=[students[4].id], notes="Hardware e integración LoRaWAN.", next_steps="Validar batería.", hours_spent=2.0, links=[{"type": "Drive", "url": "https://drive.google.com/agrosmart-minuta-1"}]),
            Meeting(group_id=groups[3].id, tutor_ids=[t_b3.id], date=datetime(2026, 4, 18, 11, 0, tzinfo=timezone.utc), participants=[students[5].id], notes="Definición de modelo pedagógico.", next_steps="Diseñar encuestas a docentes.", hours_spent=1.0, links=[]),
            Meeting(group_id=groups[4].id, tutor_ids=[t_b1.id, t_t1.id], date=datetime(2026, 4, 20, 14, 0, tzinfo=timezone.utc), participants=[students[6].id], notes="Integración de APIs bancarias.", next_steps="Revisar seguridad Open Banking.", hours_spent=3.0, links=[]),
            Meeting(group_id=groups[5].id, tutor_ids=[t_t2.id], date=datetime(2026, 4, 22, 9, 0, tzinfo=timezone.utc), participants=[students[7].id], notes="Smart contracts de transferencia solar.", next_steps="Desplegar en Testnet.", hours_spent=2.0, links=[]),
            Meeting(group_id=groups[6].id, tutor_ids=[t_b3.id], date=datetime(2025, 10, 10, 15, 0, tzinfo=timezone.utc), participants=[students[8].id], notes="Certificaciones de compostabilidad.", next_steps="Enviar muestras a laboratorio.", hours_spent=1.5, links=[]),
            Meeting(group_id=groups[7].id, tutor_ids=[t_b1.id], date=datetime(2025, 10, 12, 16, 0, tzinfo=timezone.utc), participants=[], notes="Pruebas de sensores térmicos.", next_steps="Ajustar alertas por SMS.", hours_spent=2.0, links=[]),
            Meeting(group_id=groups[8].id, tutor_ids=[t_b2.id], date=datetime(2023, 5, 14, 11, 0, tzinfo=timezone.utc), participants=[], notes="Cierre de proyecto y métricas finales.", next_steps="Presentación a autoridades.", hours_spent=1.0, links=[]),
            Meeting(group_id=groups[9].id, tutor_ids=[t_t2.id], date=datetime(2026, 4, 25, 10, 0, tzinfo=timezone.utc), participants=[students[9].id], notes="Escaneo de vulnerabilidades inicial.", next_steps="Priorizar parches críticos.", hours_spent=2.5, links=[]),
            Meeting(group_id=groups[10].id, tutor_ids=[t_b4.id], date=datetime(2026, 4, 26, 14, 0, tzinfo=timezone.utc), participants=[students[10].id], notes="Revision de algoritmos nutricionales.", next_steps="Validación con médicos.", hours_spent=1.5, links=[]),
            Meeting(group_id=groups[11].id, tutor_ids=[t_b5.id], date=datetime(2026, 4, 27, 9, 0, tzinfo=timezone.utc), participants=[students[11].id], notes="Pruebas de calidad de agua.", next_steps="Ajuste de filtros.", hours_spent=2.0, links=[]),
            Meeting(group_id=groups[12].id, tutor_ids=[t_b1.id], date=datetime(2026, 4, 28, 15, 0, tzinfo=timezone.utc), participants=[students[12].id], notes="Red de comercios adheridos.", next_steps="Firmar convenios.", hours_spent=1.5, links=[]),
            Meeting(group_id=groups[13].id, tutor_ids=[t_t2.id], date=datetime(2024, 4, 10, 11, 0, tzinfo=timezone.utc), participants=[students[13].id], notes="Entrenamiento de modelo de visión.", next_steps="Probar camaras infrarrojas.", hours_spent=3.0, links=[]),
            Meeting(group_id=groups[14].id, tutor_ids=[t_t3.id], date=datetime(2024, 9, 15, 16, 0, tzinfo=timezone.utc), participants=[students[14].id], notes="Rutas de vuelo de drones.", next_steps="Permisos de ANAC.", hours_spent=2.0, links=[]),
        ]
        db.add_all(meetings)
